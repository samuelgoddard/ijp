  ...
  return (
    <div className={`block w-full group transform ${ inverse ? '' : 'translate-x-3 md:translate-x-0' }`}>
      ...
      <div className={`overflow-hidden mb-4 transform ${ inverse ? '-translate-x-3 md:translate-x-0' : '' }`}>
        <div className="transform group-hover:scale-[1.05] group-focus:scale-[1.05] transition-transform ease-in-out duration-[750ms]">
          <ImageWrapper
            image={imageSrc} 
            layout="responsive"
      ...
      <span className="block font-sans text-2xl md:text-2xl xl:text-2xl tracking-tighter leading-none mb-1 group-hover:text-red group-focus:text-red transition-colors ease-in-out duration-500">{title}</span>
